...
const ChatMessage = memo(({ msg }) => (
  <div className="text-sm break-words leading-relaxed group hover:bg-neutral-900/50 -mx-2 px-2 py-1 rounded transition-colors">
    <span
      className={`font-bold ${msg.color || 'text-neutral-400'} mr-2 cursor-pointer hover:underline text-xs uppercase tracking-wide opacity-90`}
    >
      {msg.user}:
    </span>
...
export default function Chat({ streamId, isInline = false }) {
  ...
  const [messages, setMessages] = useState([
    {
      id: 'welcome',
      user: 'System',
      text: 'Welcome to the chat! Connect to the swarm.',
  ...
  return (
    <div
      className={
        isInline
          ? 'flex flex-col h-full bg-neutral-950 w-full relative'
          : 'flex flex-col h-full bg-neutral-950 border-l border-neutral-800 w-80 fixed right-0 top-16 bottom-0 z-40 hidden lg:flex shadow-xl'
      }
    >
      <div
        className={`p-3 border-b border-neutral-800 font-bold text-center uppercase tracking-wider text-xs text-neutral-500 bg-neutral-900/50 backdrop-blur-sm flex justify-between px-4 ${isInline ? 'rounded-t-xl' : ''}`}
      >
        <span>Stream Chat</span>
        <span
          className={`w-2 h-2 rounded-full ${isConnected ? 'bg-green-500' : 'bg-red-500'} self-center`}
      ...
      <form
        ...
        className="p-4 border-t border-neutral-800 bg-neutral-900/80 backdrop-blur-md"
      >
