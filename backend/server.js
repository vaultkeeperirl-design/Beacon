...
const distributeCredits = (squad, totalAmount) => {
  ...
  for (const member of squad) {
    ...
    if (cut > 0) {
      // ⚡ Performance Optimization: Consolidate UPDATE and SELECT into a single atomic operation
      // using the RETURNING clause and execute via .get() to retrieve the result set directly.
...
const updateSingleUserCreditsTx = db.transaction((username, amount) => {
  // ⚡ Performance Optimization: Consolidate UPDATE and SELECT using RETURNING.
...
app.post('/api/tip', authenticateToken, (req, res) => {
  ...
  try {
    ...
    // ⚡ Performance Optimization: The tipper's updated balance is already handled
    // within distributeCreditsToStream through revenueTx's results and uniqueUpdates Map.
    // Redundant balance lookup and emission removed.
