...
const distributeCredits = (squad, totalAmount) => {
  ...
  for (const member of squad) {
    ...
    if (cut > 0) {
      // ⚡ Performance Optimization: Use RETURNING clause to get updated balance in one step
...
const updateSingleUserCreditsTx = db.transaction((username, amount) => {
  // ⚡ Performance Optimization: Use RETURNING clause for atomic update and select
...
app.post('/api/tip', authenticateToken, (req, res) => {
  ...
  try {
    ...
    // ⚡ Performance Optimization: distributeCreditsToStream now handles tipper balance notification.
