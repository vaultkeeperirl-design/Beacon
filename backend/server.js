...
// ⚡ Performance Optimization: Using RETURNING credits allows us to get the updated balance
// in a single atomic operation, eliminating a subsequent SELECT query.
...
const distributeCredits = (squad, totalAmount) => {
  ...
  for (const member of squad) {
    ...
    if (cut > 0) {
      // ⚡ Performance Optimization: Atomic update with RETURNING
...
app.post('/api/tip', authenticateToken, (req, res) => {
  ...
  try {
    ...
    // ⚡ Performance Optimization: distributeCreditsToStream now handles the tipper's
    // wallet update emission automatically via revenueTx's RETURNING clause.
