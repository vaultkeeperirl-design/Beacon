...
/**
 * Checks if there is another authenticated socket for the same broadcaster in the room.
 * This ensures the stream stays alive if the host has multiple tabs/sessions.
 * @param {string} streamId - The ID of the stream (which is also the broadcaster's username).
 * @param {string} currentSocketId - The ID of the socket that is currently disconnecting/leaving.
...
io.on('connection', (socket) => {
  ...
  // 🛡️ SECURITY: Initialize as unauthenticated
  ...
  socket.on('join-stream', (data) => {
    ...
    // 🛡️ SECURITY: Prioritize authenticated identity (socket.accountName)
    // to prevent spoofing via client-supplied data. Unauthenticated users are null.
    const accountName = socket.accountName || null;
    let username = accountName || 'Guest';
    ...
    if (accountName) {
      ...
      // Anyone else claiming to be the host (but not authenticated as such) gets a fallback name.
    ...
    if (socket.currentRoom && socket.currentRoom !== streamId) {
       ...
       // 🌉 Bridge: Only end stream if this is the LAST active broadcaster session for this room.
       if (socket.isAuthenticated && socket.accountName === prevRoom && activeStreams.has(prevRoom)) {
  ...
  socket.on('leave-stream', () => {
    if (socket.currentRoom) {
       ...
       // activeStreams logic: If host leaves, redirect viewers
       // 🌉 Bridge: Only end stream if this is the LAST active broadcaster session for this room.
       if (socket.isAuthenticated && socket.accountName === room && activeStreams.has(room)) {
  ...
  socket.on('update-squad', ({ streamId, squad }) => {
    ...
    if (!socket.isAuthenticated || socket.accountName !== streamId) return;
  ...
  socket.on('disconnect', () => {
    ...
    if (streamId) {
      // activeStreams logic: If host leaves, redirect viewers
      // 🌉 Bridge: Only end stream if this is the LAST active broadcaster session for this room.
      if (socket.isAuthenticated && socket.accountName === streamId && activeStreams.has(streamId)) {
...
module.exports = { server, io, streamSquads, JWT_SECRET, isAnotherBroadcasterActive };
