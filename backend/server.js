...
// ⚡ Performance Optimization: Using RETURNING clause (SQLite 3.35.0+) to combine
// UPDATE and SELECT into a single atomic operation, reducing DB round-trips by 50%.
...
const distributeCredits = (squad, totalAmount) => {
  ...
  for (const member of squad) {
    ...
    if (cut > 0) {
      // ⚡ Performance Optimization: Using RETURNING to avoid separate SELECT
...
const revenueTx = db.transaction((tipper, amount, squad, relayers, relayTotal, squadTotal) => {
  ...
  if (tipper) {
    // ⚡ Performance Optimization: Merge balance check, deduction, and fetch into a single atomic UPDATE.
    const row = deductCreditsWithCheckStmt.get(amount, tipper, amount);
    if (!row) {
      throw new Error('INSUFFICIENT_FUNDS');
    }
    // Include tipper update for socket emission
...
app.post('/api/tip', authenticateToken, (req, res) => {
  ...
  try {
    ...
    // ⚡ Performance Optimization: distributeCreditsToStream now returns all updates,
    // including the tipper's new balance, via the revenueTx transaction results.
