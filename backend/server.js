...
const distributeCredits = (squad, totalAmount) => {
  ...
  for (const member of squad) {
    ...
    if (cut > 0) {
      // ⚡ Performance Optimization: Use .get() with RETURNING to get updated balance in one round-trip.
...
const updateSingleUserCreditsTx = db.transaction((username, amount) => {
  // ⚡ Performance Optimization: Use .get() with RETURNING for atomic update and select.
...
app.post('/api/tip', authenticateToken, (req, res) => {
  ...
  try {
    ...
    // ⚡ Performance Optimization: distributeCreditsToStream now handles tipper balance emission.
