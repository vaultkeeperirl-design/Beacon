...
const distributeCredits = (squad, totalAmount) => {
  ...
  for (const member of squad) {
    ...
    if (cut > 0) {
      // ⚡ Performance Optimization: Use RETURNING clause to get updated credits in one round-trip.
...
const revenueTx = db.transaction((tipper, amount, squad, relayers, relayTotal, squadTotal) => {
  if (tipper) {
    // ⚡ Performance Optimization: Merge balance check, deduction, and fetch into a single atomic operation.
