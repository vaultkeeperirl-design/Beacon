...
// in a single database call, halving the number of queries for credit-heavy operations.
...
const distributeCredits = (squad, totalAmount) => {
  ...
  for (const member of squad) {
    ...
    if (cut > 0) {
      // ⚡ Performance Optimization: Using .get() with RETURNING clause instead of .run() + SELECT
...
const updateSingleUserCreditsTx = db.transaction((username, amount) => {
  // ⚡ Performance Optimization: Using .get() with RETURNING clause
...
const revenueTx = db.transaction((tipper, amount, squad, relayers, relayTotal, squadTotal) => {
  if (tipper) {
    // ⚡ Performance Optimization: Merge balance check, deduction, and fetch into a single atomic UPDATE ... RETURNING.
