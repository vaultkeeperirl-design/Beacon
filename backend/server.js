...
// ⚡ Performance Optimization: Using RETURNING clause (SQLite 3.35.0+) to combine UPDATE and SELECT.
...
const distributeCredits = (squad, totalAmount) => {
  ...
  for (const member of squad) {
    ...
    if (cut > 0) {
      // ⚡ Performance Optimization: Atomic UPDATE with RETURNING clause
      // This eliminates the need for a separate SELECT query to get the updated balance.
...
const revenueTx = db.transaction((tipper, amount, squad, relayers, relayTotal, squadTotal) => {
  ...
  if (tipper) {
    // ⚡ Performance Optimization: Atomic balance check, deduction, and fetch in one operation.
    const row = deductCreditsWithCheckStmt.get(amount, tipper, amount);
    if (!row) {
      throw new Error('INSUFFICIENT_FUNDS');
    }
    // Include the updated tipper balance in results for deduplicated notification
...
const distributeCreditsToStream = (streamId, amount, tipper = null) => {
  ...
  try {
    // ⚡ Performance Optimization: revenueTx now includes the tipper's updated balance
    // in the 'updates' array, allowing for a single unified emission pass.
...
app.post('/api/tip', authenticateToken, (req, res) => {
  ...
  try {
    ...
    // ⚡ Performance Optimization: distributeCreditsToStream now returns the updated
    // balance for the tipper and all recipients, and handles wallet-update emissions
    // in a single deduplicated pass.
