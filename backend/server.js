...
// ⚡ Performance Optimization: Using RETURNING clause allows combined UPDATE and SELECT
// in a single atomic database operation, reducing round-trips and improving performance.
...
const distributeCredits = (squad, totalAmount) => {
  ...
  for (const member of squad) {
    ...
    if (cut > 0) {
      // ⚡ Performance Optimization: Combined UPDATE and SELECT using RETURNING clause.
...
const revenueTx = db.transaction((tipper, amount, squad, relayers, relayTotal, squadTotal) => {
  const results = [];
  if (tipper) {
    // ⚡ Performance Optimization: Merge balance check, deduction, and SELECT into a single atomic operation.
...
app.post('/api/tip', authenticateToken, (req, res) => {
  ...
  try {
    ...
    // ⚡ Performance Optimization: Wallet update for the tipper is now handled inside
    // distributeCreditsToStream via revenueTx's RETURNING clause and consolidated emissions.
